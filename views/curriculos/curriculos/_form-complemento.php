    ...
    <?php foreach ($modelsComplementos as $i => $modelComplemento): ?>
        <div class="complemento panel panel-default"><!-- widgetBody -->
            ...
            <div class="panel-body">
                ...
                ?>
                <div class="row">
                    <div class="col-sm-2"><?= $form->field($modelComplemento, "[{$i}]certificado")->radioList([1 =>'Sim', 0 =>'Não'], ['inline'=>true]) ?></div>

                    <div class="col-sm-4"><?= $form->field($modelComplemento, "[{$i}]cursos")->textInput(['placeholder'=>'Informe seu curso...', 'maxlength' => true]) ?></div>

                    <div class="col-sm-4"><?= $form->field($modelComplemento, "[{$i}]local")->textInput(['placeholder'=>'Informe a Instituição...', 'maxlength' => true]) ?></div>

                    <div class="col-sm-2"><?= $form->field($modelComplemento, "[{$i}]carga_horaria")->textInput(['placeholder'=>'Informe a carga horária...','maxlength' => true]) ?></div>
                </div><!-- .row -->
            </div>
        </div>
    <?php endforeach; ?>
