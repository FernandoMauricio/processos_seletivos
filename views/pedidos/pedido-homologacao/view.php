...
<div class="pedido-homologacao-view" style="margin-left: -160px;margin-right: -160px;">
...
<table width="100%" border="0">
  <tr> 
    ...
    <td width="60%"><h7>SERVIÇO NACIONAL DE APRENDIZAGEM COMERCIAL - SENAC</h7><br />
                    ...
                    <h7>PEDIDO DE HOMOLOGAÇÃO PROCESSO SELETIVO Nº <?= $model->etapasProcesso->etapa_id . '/' . date('Y', strtotime($model->homolog_data)); ?></h7><br />
                        <?= strftime('%A, %d de %B de %Y', strtotime($model->homolog_data)); ?>
    </td>
    <td width="20%"><b>Homologação Nº <?= $model->homolog_id . '/' . date('Y', strtotime($model->homolog_data)) ?>
  </tr>
</table>
  ...
  <table class="table table-condensed table-hover">
     <tbody>
        <?php if($model->homolog_situacaoggp == 1){ ?>
          <td style="font-size: 10px; border-top: 0px solid"><b><span class="glyphicon glyphicon-lock" aria-hidden="true"> </span> <?= $model->homologSituacaoggp->situacao_descricao ?><br /><br /></td>
          <?php }else{?>
          <?php echo $model->homolog_situacaoggp == 3 ? '<td style="font-size: 10px; border-top: 0px solid"><b><span class="glyphicon glyphicon-remove" aria-hidden="true"> </span>' : '<td style="font-size: 10px; border-top: 0px solid"><b><span class="glyphicon glyphicon-ok" aria-hidden="true"> </span>'; ?>
            <?= $model->homologSituacaoggp->situacao_descricao ?></b><br /><br /><br /><br />
            ...
            <?= isset($query['aprov_observacao']) ? $query['aprov_observacao'] . '<br />' : false ?>
            <?php echo date('d/m/Y', strtotime( $model->homolog_dataaprovacaoggp )) ?>&nbsp;&nbsp;&nbsp;<br />
          </td>
          <?php }?>

        <?php if($model->homolog_situacaodad == 1){ ?>
          <td style="font-size: 10px; border-top: 0px solid"><b><span class="glyphicon glyphicon-lock" aria-hidden="true"> </span> <?= $model->homologSituacaodad->situacao_descricao ?><br /><br /></td>
          <?php }else{?>
          <?php echo $model->homolog_situacaodad == 3 ? '<td style="font-size: 10px; border-top: 0px solid"><b><span class="glyphicon glyphicon-remove" aria-hidden="true"> </span>' : '<td style="font-size: 10px; border-top: 0px solid"><b><span class="glyphicon glyphicon-ok" aria-hidden="true"> </span>'; ?>
            <?= $model->homologSituacaodad->situacao_descricao ?></b><br /><br /><br /><br />
            Assinado eletrônicamente por:<br />
